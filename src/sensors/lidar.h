...
struct Lidar
{
	...
	{
		minDistance = 5; //set min distance so that lidar can avoid car rooftop pcd
		maxDistance = 50;
		resoultion = 0.2;
		sderr = 0.2; //noise of lidar
		cars = setCars;
		groundSlope = setGroundSlope;
		int numLayers = 8; //gives point cloud resolution
		double steepestAngle =  30.0*(-pi/180);
		double angleRange = 26.0*(pi/180);
		double horizontalAngleInc = pi/64; //decrease horizontal layer increment
	...
	{
		//generates a point cloud and does ray casting
		cloud->points.clear();
