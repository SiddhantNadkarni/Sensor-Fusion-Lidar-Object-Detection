...
void Proximity(int indice, std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& pointMark, KdTree<std::vector<float>>* tree, float distanceTol)
{

  pointMark[indice] = 1;
  cluster.push_back(indice);
  std::vector<int> nearbyPoints = tree->search(points[indice], distanceTol); //returns list of indices that are nearby indice
  for (int i = 0; i < nearbyPoints.size(); ++i)
  {
    if(!pointMark[nearbyPoints[i]])
      Proximity(nearbyPoints[i], points, cluster, pointMark, tree, distanceTol);
  }
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree<std::vector<float>>* tree, float distanceTol)
{
  ...
  std::vector<bool> pointMark(points.size(), 0);
  for (int i = 0; i < points.size(); ++i)
  {
    if (!pointMark[i])
    {
      std::vector<int> cluster;
      Proximity(i, points, cluster, pointMark, tree, distanceTol);
...
int main ()
{
	...
	KdTree<std::vector<float>>* tree = new KdTree<std::vector<float>>();
